// Exercise 5.16
// Add the -d ("directory order") option, which makes comparisons onlye on
...
int strcmp_lower(const char *s, const char *t);
int is_directory(int c);

static int numeric = 0, reverse = 0, fold = 0, directory = 0;
...
/* is_directory: return true if character is alphanumeric, number or blank */
int is_directory(int c){
  return isalpha(c) || isdigit(c) || c == ' ';
}

/* dircmp: compare with directory rules */
int dircmp(const char *s, const char *t){
  while(*s || *t){
    while(*s && !is_directory((unsigned char)*s))
      s++;
    while(*t && !is_directory((unsigned char)*t))
      t++;
...
/* strcmp_lower: folds both strings to lowercase for comparison */
int strcmp_lower(const char *s, const char *t){
  int a, b;
  
  for( ; *s && *t; ++s, ++t){
    a = tolower((unsigned char)*s);
    b = tolower((unsigned char)*t);
